...
class Game:
    ...
    def update_projectiles(self):
        ...
        for bullet in self.player.bullets[:]:
            ...
            if (bullet["x"] < 0 or bullet["x"] > pyxel.width or
    ...
    def check_collisions(self):
        ...
        for ball in self.player.energy_balls:
            for enemy in self.enemies[:]:
                if (abs(ball["x"] - enemy.x) < ball["size"] + enemy.size and
                    abs(ball["y"] - enemy.y) < ball["size"] + enemy.size):
                    enemy.health -= 0.1
                    if enemy.health <= 0:
                        self.enemies.remove(enemy)
                        self.player.score += 2 if enemy.type == "elite" else 1
    ...
    def draw(self):
        ...
        pyxel.tri(
            ...
            self.player.x + self.player.size, self.player.y + self.player.size/2,
        ...
        for effect in self.effects:
            if effect["type"] == "hit":
                pyxel.circb(effect["x"], effect["y"], 
                           10 - effect["timer"], 10)  # 黄色
